...
#remove first two row, because un-revelent
UO_T1_ders = UO_T1_ders[-c(1,2),]
UO_T2_ders = UO_T2_ders[-c(1,2),]
UO_T3_ders = UO_T3_ders[-c(1,2),]
UO_T4_ders = UO_T4_ders[-c(1,2),]

UPMC_T1_ders = UPMC_T1_ders[-c(1,2),]
UPMC_T2_ders = UPMC_T2_ders[-c(1,2),]
UPMC_T3_ders = UPMC_T3_ders[-c(1,2),]
UPMC_T4_ders = UPMC_T4_ders[-c(1,2),]
```
...
DERS_Prep<- merge(DERS_Qualtrics, Pedigree_Prep, all = TRUE)
...
Progress <- select(DERS_Prep, c(FamID, GroupAssignment, Timepoint, starts_with("srm")))

# Calculate row mean for each individual  
Progress$Row_Mean <- rowMeans(select(DERS_Prep,c(starts_with("srm"))), na.rm = T)
...
Mean_Table <- data.frame(UO[,c("UO_DERS","GroupAssignment")],UPMC[,"UPMC_DERS"],Both_site_Mean[,c("BothSiteMean","n")])
