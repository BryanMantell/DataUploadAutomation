...
```{r Column Names Variable, include=FALSE}
...
# Check NA percentage
CCNES_PREP$NACheck <- rowSums(is.na(dplyr::select(CCNES_PREP, starts_with("srm"))))/ncol(dplyr::select(CCNES_PREP, starts_with("srm")))
...
```{r NDA sheet output, include=FALSE}
...
```{r treatment mean, include=FALSE}
