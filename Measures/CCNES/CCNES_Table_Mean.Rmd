...
Progress$Supported_Mean <- rowMeans(CCNES_PREP[,c(EE,EFR,PFR)], na.rm = TRUE)
Progress$Unsupproted_Mean <- rowMeans(CCNES_PREP[,c(DR, PR, MR)], na.rm = TRUE)
...
Progress_Supported_Mean_both <- Progress %>%
  ...
  summarise("Both Site Supported Mean" = mean(Supported_Mean,na.rm = T), n = n())

Progress_unsupported_Mean_both <- Progress %>%
  group_by(GroupAssignment,Timepoint) %>%
  summarise("Both Site Unsupproted Mean" = mean(Unsupproted_Mean,na.rm = T), n = n())
...
colnames(UO)[colnames(UO) == "SupportedMean"] = "UO_SupportedMean"
colnames(UO)[colnames(UO) == "UnsupprotedMean"] = "UO_UnsupprotedMean"
colnames(UPMC)[colnames(UPMC) == "SupportedMean"] = "UPMC_SupportedMean"
colnames(UPMC)[colnames(UPMC) == "UnsupprotedMean"] = "UPMC_UnsupprotedMean"


# Combine UO,UPMC and both_site data 
Mean_Table <- data.frame(UO[,c("UO_SupportedMean","UO_UnsupprotedMean")],UPMC[,c("UPMC_SupportedMean","UPMC_UnsupprotedMean")],Both_site_Mean[,c("Both Site Supported Mean","Both Site Unsupproted Mean","n","GroupAssignment")])
