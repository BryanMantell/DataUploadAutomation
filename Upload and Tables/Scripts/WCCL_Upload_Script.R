...
#r Recode
#Change Strings of Text to Numerical
...
#Change Desired Columns to Numeric values
...
WCCL_NDA_Prep <- select(WCCL_Prep, c(visit = Timepoint, subjectkey = mom_guid, src_subject_id = FamID_Mother, sex = mother_sex, interview_date, interview_age = interview_age_Mom, dbt_wccl_su = WCCL_SU_imputation, dbt_wccl_gdc = WCCL_GSC_imputation, dbt_wccl_bo = WCCL_BO_imputation, contains("srm_wccl")))
...
# NDA output ####
write.table(first_line, file = "NDA Upload/dbt_wccl.csv", sep = ",", append = FALSE, quote = FALSE, na = "", col.names = FALSE, row.names = FALSE)

# Append data in WCCL_NDA into dbt_wccl.cav file 
write.table(WCCL_NDA, file = 'NDA Upload/dbt_wccl.csv', sep = ",", append = TRUE, na = "", quote = FALSE, row.names = FALSE)
